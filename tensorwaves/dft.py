...
# def get_projected_potential_slice(origin, extent):


# class ProjectedPAWPotentialCorrections(HasGrid):
#
#     def __init__(self, extent=None, gpts=None, sampling=None, quadrature='riemann', num_samples=54):
#
...
#         return np.sum(func(rxy) * wkab.reshape(1, -1), axis=-1)
#
#     def get_slice(self, positions, funcs, slice_entrance, slice_exit):
#
#         def get_indices_in_slice(positions, slice_entrance, slice_exit, r_cut):
#             return np.where(((slice_entrance - r_cut) < positions[:, 2]) &
#                             ((slice_exit + r_cut) > positions[:, 2]))[0]
#
#         margin = np.ceil(self._rmax / units.Bohr / min(self.sampling)).astype(np.int32)
#         padded_gpts = self.gpts + 2 * margin
#
#         v = np.zeros(padded_gpts)
#
#         for i in get_indices_in_slice(positions, slice_entrance, slice_exit):
#             func = funcs[i][0]
#             position = positions[i]
#
#             rc = func.get_cutoff()
...
#             b = slice_exit - position[2]
#
#             nodes = np.linspace(0., rc, 200) / units.Bohr
#
#             radial = self.project(func.map, nodes, a / units.Bohr, b / units.Bohr)
#
#             corner_index = (np.round(position[:2] / self.sampling)).astype(np.int) - block_margin + margin
#
#             position_in_block = position[:2] + self.sampling * margin - self.sampling * corner_index
#
...
#             r_interp = np.sqrt(((x - position_in_block[0]) ** 2)[:, None] +
#                                ((y - position_in_block[1]) ** 2)[None, :]) / units.Bohr
#             r_interp = np.clip(r_interp, 0, rc / units.Bohr)
#
#             f = interp1d(nodes, radial)
#             v_interp = f(r_interp)
#             v[corner_index[0]: corner_index[0] + block_size, corner_index[1]: corner_index[1] + block_size] += v_interp
